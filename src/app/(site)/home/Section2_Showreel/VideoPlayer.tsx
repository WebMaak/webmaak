...
import "@/styles/showreel/VideoPlayer.css";
...
const VideoPlayer: React.FC<VideoPlayerProps> = ({
  ...
  const [loadVideo, setLoadVideo] = useState(false); // Facade state

  const containerRef = useRef<HTMLDivElement>(null);
  const iframeRef = useRef<HTMLIFrameElement>(null);

  const isActuallyLoading = isLoading || (loadVideo && !iframeLoaded);

  // Shared animation transition for synchronization
  const overlayTransition: any = { duration: 0.4, ease: [0.23, 1, 0.32, 1] };

  useEffect(() => {
    // Defer loading video until it's in view
    if (isInView && !loadVideo) {
      setLoadVideo(true);
    }
  }, [isInView, loadVideo]);
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      try {
        ...
        // Handle loaded/ready event for duration and loading state
        if (data.event === "loaded" || data.event === "ready") {
  ...
  useEffect(() => {
    ...
    setCurrentTime(0);
    // Don't reset loadVideo here if we want to stay in 'iframe' mode 
    // unless we need to force reload the iframe, but the parent key handles that.
  }, [project.id]);
  ...
  return (
    <div
      ref={containerRef}
      className={`player-wrapper ${isMobile ? "mobile-mode" : ""}`}
      ...
      <div className="video-viewport">
        {loadVideo ? (
          ...
          <div className="video-facade">
            {/* Fallback facade - using logo as a placeholder or project specific thumbnail if available */}
            <div className="facade-content">
              <Image
                ...
                width={200}
                height={100}
                className="facade-logo"
                style={{ objectFit: "contain" }}
              />
              <div className="facade-play-hint">
      ...
      {isMobile && !isActuallyLoading && (
        <div className="mobile-slim-controls">
          ...
          <div className="slim-right">
            ...
            <button
              aria-label={isMobile ? "Fullscreen" : "Exit fullscreen"}
